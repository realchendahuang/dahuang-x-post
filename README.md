...
1. **先诊断，不急着写**——原稿最硬的判断是什么？哪句适合做第一行？哪些背景可以删？读者为什么会停下来看？对潜质有疑问时补读 `viral-post-anatomy.md`，看 7 特征命中几个、爆款≠涨粉有没有踩坑。
2. **抽一个可传播判断**——先打至少 3 个开头，选最硬的一条交付（见 Hook 矩阵）。
3. **选形态**——默认短帖；3 个以上独立观点或步骤/证据链才写 Thread。
4. **改成 X 信息流结构**——一行一句，每行只推进一个信息，删掉铺垫型开头，链接直接放帖子里。

### 出厂自检

第一行有没有判断、前 3 行交代 who/what/why 没、有没有具体细节、是不是一行一句、有没有自然互动点、字符数够不够。
...
│   ├── viral-post-anatomy.md         # 爆款潜质判断（核心判断句+7特征+注意事项+公式）
