...
先打至少 3 个开头。默认把这 3 个开头作为「备选开头」随帖子一起交付，让用户选第一句。
...
## 长版（Premium Longer post，不卡字数）
{同一条帖的展开版：判断不变，证据/细节/场景铺开，链接直接在里面}
...
- 短版和长版是同一条帖的两种长度，不是两条不同内容。
- 短版优先保证第一行判断 + 一个证据能站住，砍掉的不是判断，是铺垫。
- 长版不为了长而长，只把短版里被迫删掉的具体细节补回来。
- 3 个备选开头只换第一句，正文不动。给用户选第一句用。
- 链接在两版里都直接放帖子正文，不单独列。
...
Thread 每条按短版字符意识写（能短则短），但不硬卡 280——优先每条一个独立 idea。
...
用户说"只要一条"或"不用给长版"时，只给短版，不给长版和备选开头。不自作主张加。
...
- 是否一行一句，扫读无阻力？
- 是否有自然互动点，不是泛泛问"你怎么看"？
- 短版是否在 280 字符内？长版不卡字数，但每行是否仍只推进一个信息？
