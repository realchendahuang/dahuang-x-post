...
默认输出长短两版（短版 ≤280 字符给普通账号，长版不卡字数给 Premium Longer post）+ 3 个备选开头。用户按自己账号权限选发哪个。
...
2. **抽一个可传播判断**——先打至少 3 个开头，作为备选开头随帖子交付（见 Hook 矩阵）。
3. **选形态**——默认短帖；3 个以上独立观点或步骤/证据链才写 Thread。
4. **改成 X 信息流结构**——一行一句，每行只推进一个信息，删掉铺垫型开头，链接直接放帖子里。

### 出厂自检

第一行有没有判断、前 3 行交代 who/what/why 没、有没有具体细节、是不是一行一句、有没有自然互动点、短版在不在 280 字符内。
...
默认就给长短两版 + 3 个备选开头，不用额外指定。想只要一版：

```
Use $dahuang-x-post 把下面这段改成 X 帖子，只要短版，不用给长版和备选开头：
